...
export async function invokeN8nWaBookingStart(cartItemId: string, accessToken: string) {
...
export async function startN8nWaBooking(cartItemId: string, accessToken: string): Promise<{ ok: true } | { ok: false; message: string }> {
  const { data, error } = await invokeN8nWaBookingStart(cartItemId, accessToken);
