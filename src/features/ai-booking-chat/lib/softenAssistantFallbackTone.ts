...
const NEGATIVE_CLAUSE_REPLACEMENTS: Array<{ pattern: RegExp; replace: string | ((...args: string[]) => string) }> = [
  ...
  {
    pattern: /\bI couldn'?t find[^.!?]+[.!?]\s*/gi,
    replace: "",
  },
  {
    pattern: /\bI didn'?t find[^.!?]+[.!?]\s*/gi,
    replace: "",
  },
  {
    pattern: /\bThere (?:is|are) no[^.!?]+[.!?]\s*/gi,
    replace: "",
  },
  {
    pattern: /\bNothing (?:in the list )?matches?[^.!?]+[.!?]\s*/gi,
    replace: "",
  },
  {
  ...
  {
    pattern: /\b(?:unfortunately|sadly),?\s*/gi,
    replace: "",
  },
  {
    pattern: /\b(?:я )?не наш[её]л[^.!?]+[.!?]\s*/giu,
    replace: "",
  },
  {
    pattern: /\bне нашлось[^.!?]+[.!?]\s*/giu,
    replace: "",
  },
  {
    pattern: /\bни одн[аоую][^.!?]+не[^.!?]+[.!?]\s*/giu,
    replace: "",
  },
  {
...
/** Rewrites blunt "no matches" phrasing into warm, suggestive concierge tone. */
