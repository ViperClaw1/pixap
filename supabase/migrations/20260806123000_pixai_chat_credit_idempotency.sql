...
create unique index if not exists booking_credit_ledger_ai_request_idx
  on public.booking_credit_ledger (user_id, request_id)
  where request_id is not null;

drop function if exists public.consume_ai_query_credit(uuid, numeric, int, int);

create function public.consume_ai_query_credit(
  p_user_id uuid,
  p_delta numeric,
...
begin
  ...
  if exists (
    ...
    return jsonb_build_object('ok', true, 'balance', -1, 'charged', 0, 'deduplicated', false);
  end if;
...
revoke all on function public.consume_ai_query_credit(uuid, numeric, int, int, uuid) from public;
grant execute on function public.consume_ai_query_credit(uuid, numeric, int, int, uuid) to service_role;
