-- Admin dashboard aggregates (staff only: profiles.account_role = admin).
-- Callable by authenticated clients; enforces role inside SECURITY DEFINER function.

create or replace function public.assert_admin_analytics_access()
returns void
language plpgsql
security definer
...
revoke all on function public.assert_admin_analytics_access() from public;

-- Estimated monthly price in cents (USD) for MRR — update when store prices change.
-- pixai_premium_monthly / pixai_premium_annual match default SKUs in app config.
...
begin
  ...
  -- Subscriptions (IAP transactions)
  select count(*)::int into v_sub_total
  from public.subscription_transactions st
  where st.status in ('purchased', 'renewed', 'trialing')
  ...
  -- MRR estimate: active entitlements × list price (cents)
  select coalesce(sum(
    case
      when se.product_id = 'pixai_premium_annual' then 9999 / 12
      when se.product_id = 'pixai_premium_monthly' then 1299
      else 1299
    end
  ...
  -- Flat MRR series (snapshot at period end — daily history needs entitlement snapshots later)
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'date', to_char(d.day, 'YYYY-MM-DD'),
...
comment on function public.admin_analytics_summary(integer) is
  'Staff-only dashboard metrics. Requires profiles.account_role = admin for auth.uid().';
