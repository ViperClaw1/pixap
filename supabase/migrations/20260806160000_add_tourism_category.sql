-- Browse-only Tourism category used by the app and business-card seed tooling.

insert into public.categories (id, name)
select 'c7feef80-2984-49fd-85f1-d615f5a3ce9c'::uuid, 'Tourism'
where not exists (
  select 1
  from public.categories
  where lower(trim(name)) = 'tourism'
)
...
comment on function public.business_card_category_pool(uuid) is
  'Maps categories.id to seed photo pool key for localized description templates.';
...
begin
  ...
  if pool = 'tourism' then
    case loc
      when 'ru' then return format('«%s» — достопримечательность в %s. История, культура и знаковые виды города.', n, city_short);
      when 'es' then return format('%s: atracción en %s. Historia, cultura y vistas memorables.', n, city_short);
      when 'pt' then return format('%s — atração em %s. História, cultura e vistas memoráveis.', n, city_short);
      when 'fr' then return format('%s — attraction à %s. Histoire, culture et vues mémorables.', n, city_short);
      when 'de' then return format('%s — Sehenswürdigkeit in %s. Geschichte, Kultur und besondere Ausblicke.', n, city_short);
      else return format('%s — attraction in %s. Local history, culture, and memorable views.', n, city_short);
    end case;
  elsif pool = 'bar' then
    case loc
