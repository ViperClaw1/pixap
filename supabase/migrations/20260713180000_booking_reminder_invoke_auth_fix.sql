-- Align booking reminder / email pg_net invoke auth with new Supabase secret keys
-- (same pattern as generate-daily-recommendations).
...
begin
  ...
  if reminder_secret is not null then
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'x-reminder-cron-secret', reminder_secret
    );
    ...
    raise warning '[booking-reminder] invoke skipped: set vault secret reminder_cron_secret or service key (REMINDER_CRON_SECRET should match Edge REMINDER_CRON_SECRET)';
    return null;
  end if;
