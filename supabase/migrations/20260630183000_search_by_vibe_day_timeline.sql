-- Vibe Match: add "day" timeline tag boosts for daytime venue search.
...
as $$
  ...
  timeline_tokens as (
    ...
    cross join lateral unnest(
      case x.timeline_q
        when 'day' then array[
          'cafes', 'restaurants', 'brunch', 'casual', 'aesthetic', 'cozy', 'calm',
          'lounges', 'rooftop', 'wine_bars', 'social'
        ]::text[]
...
comment on function public.search_by_vibe(text, text, text, integer) is
  'Rank business_cards in p_city by mood slug tokens, user_preferences, timeline tag boosts (day/evening/night/late_night), and profiles avoid_tags.';
