-- Inbox summary: peer read cursor for outgoing read receipts in the messages list.
...
as $$
  ...
  peer_reads as (
    select
      mtp.thread_id,
      max(mtp.last_read_message_at) as peer_last_read_at
    from public.message_thread_participants mtp
