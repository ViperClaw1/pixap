-- Posts feed: server-side boost + affinity scoring + hydration (one round-trip per page).
...
grant execute on function public.get_posts_feed_page(integer, uuid, smallint, timestamptz, timestamptz, smallint, uuid) to anon, authenticated;

comment on function public.get_posts_feed_page(integer, uuid, smallint, timestamptz, timestamptz, smallint, uuid) is
  'Paginated posts feed with server-side boost + affinity scoring (following > interacted place > other).';
