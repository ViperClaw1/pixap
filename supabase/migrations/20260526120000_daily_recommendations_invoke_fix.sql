-- Align DB invoke with consume-push-outbox: prefer push_cron_secret, fallback service_role_key.
...
begin
  ...
  if cron_secret is not null then
    ...
    raise warning '[daily-recs] invoke skipped: set vault secret service_role_key or push_cron_secret (same value as Edge PUSH_CRON_SECRET)';
    return null;
  end if;
