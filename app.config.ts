...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  const appVersion = process.env.APP_VERSION?.trim() ?? config.version ?? "1.0.3";
  const iosBuildNumber = process.env.IOS_BUILD_NUMBER?.trim() ?? config.ios?.buildNumber ?? "61";
  const androidVersionCodeRaw = process.env.ANDROID_VERSION_CODE?.trim() ?? String(config.android?.versionCode ?? "55");
    ...
    intentFilters: [
      {
        ...
        data: [
          ...
          { scheme: "https", host: "pixapp.kz", pathPrefix: "/bookings" },
          { scheme: "https", host: "www.pixapp.kz", pathPrefix: "/place" },
          { scheme: "https", host: "www.pixapp.kz", pathPrefix: "/post" },
          { scheme: "https", host: "www.pixapp.kz", pathPrefix: "/story" },
          { scheme: "https", host: "www.pixapp.kz", pathPrefix: "/bookings" },
        ],
