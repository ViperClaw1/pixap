...
| Railway **`delivery_status_ingested`** + **`Media upload error`** | First template (`check_is_available_en` / `_ru`) header image URL is invalid for Meta. Fix **`WHATSAPP_CHECK_IS_AVAILABLE_*_HEADER_IMAGE_URL`** on Railway; see [`backend/wa-booking-service/README.md`](../backend/wa-booking-service/README.md). `GET /health` on the service reports `header_image_checks`. |
