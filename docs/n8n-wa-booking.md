...
| Railway **`header_image_precheck_failed`** or **`Media upload error`** | A **`WHATSAPP_*_HEADER_IMAGE_URL`** is set but the URL is not a direct public image (common: Supabase Storage URL returning JSON 400). If templates use a **static** logo in Meta, **remove** those env vars on Railway — the service does not need a runtime header URL. |
