...
| Railway **`header_image_precheck_failed`** or **`Media upload error`** | Set **`WHATSAPP_HEADER_LOGO_URL`** on `wa-booking-service` to one **public HTTPS** image (for Supabase: `.../object/public/...`). All flow templates share this logo unless overridden per template. Check `GET /health` → `header_image_checks`. |
